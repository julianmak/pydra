...
real:: t1,t2,t,zfac,hzfac,avgfac
...
open(31,file='qq1.r4',form='unformatted', &
    & access='direct',status='old',recl=nbytes)
open(32,file='qq2.r4',form='unformatted', &
    & access='direct',status='old',recl=nbytes)
...
open(66,file='ext-zonal.asc',status='replace')
open(88,file='ens-u-correl.asc',status='replace')
...
do  
  loop=loop+1
  iread=0
  read(31,rec=loop,iostat=iread) t,q1r4
  if (iread .ne. 0) exit 
  read(32,rec=loop,iostat=iread) t,q2r4
  ...
  write(22,rec=loop) t,u1avg,k1avg,q1avg,z1avg,f1avg, &
  ...
  enddo
  write(66,'(f7.2,10(1x,f11.7))') & 
   & t,u1max,k1max,q1max,z1max,f1max,u2max,k2max,q2max,z2max,f2max
  ...
  write(88,'(f7.2,2(1x,f11.7))') t,corr1,corr2
...
if (timeavg) then
  ...
  open(41,file='u1avg.asc',status='replace')
  open(42,file='k1avg.asc',status='replace')
  open(43,file='q1avg.asc',status='replace')
  open(44,file='z1avg.asc',status='replace')
  open(45,file='f1avg.asc',status='replace')
  open(51,file='u2avg.asc',status='replace')
  open(52,file='k2avg.asc',status='replace')
  open(53,file='q2avg.asc',status='replace')
  open(54,file='z2avg.asc',status='replace')
  open(55,file='f2avg.asc',status='replace')
  do iy=0,ny
